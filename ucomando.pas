...
type
  ...
  TParametro= ^PParametro;
  PParametro= record
      datoString: String;
      datoNumericoEntero: integer;
      datoNumericoReal: real;
      esEntero: boolean;
      esReal: boolean;
  end;
  ...
  * De Leon
  ...
  ===================================================================}
  {Crea un nuevo parámetro con el String pasado como argumento. Si dicha cadena
  además es un número se establecerá un valor numérico para retornar como entero
  o como real.}
  function crearParametro(param: string): TParametro;

  {Retorna TRUE si el parámetro p es de tipo string NO numérico, FALSE si no.
  Esta operación es análoga a esParametroNumerico}
  function esParametroString(const p: TParametro): boolean;

  {Retorna TRUE si el parámetro p es un entero, FALSE si no.
  Esta operación es análoga a esParametroString}
  function esParametroEntero(const p: TParametro): boolean;

  {Retrona el valor numérico de p como un entero. Si p no es entero se retorna
  el valor 0.}
  function obtenerNumeroEntero(const p: TParametro): integer;

  {Retorna el valor numérico de p como un real. Si p no es un número se retorna
  el valor 0.}
  function obtenerNumeroReal(const p: TParametro): real;
  ...
  {Libera la memoria ocupada por p y lo deja en NIL. Si p ya era NIL
  no hace nada.}
  procedure eliminarParametro(var p: TParametro);
  ...
  * Si no hay comillas, el espacio en blanco se usa como separador, incluso
  ...
  NULL como String.}
  function siguienteParametro(const c: TComando): TParametro;

  {Regresa la lista de parámetros al inicio para poder recorrerla nuevamente.}
  procedure resetearParametros(const c: TComando);

  {Retorna la cantidad de parámetros disponibles para el comando en
  cuestión.}
  function cantidadParametros(const c: TComando): byte;

  {Libera la memoria ocupada con el comando c y lo deja en NIL.}
  procedure eliminarComando(var c: Tcomando);
...
begin
    new(result);
    result^.datoString:= param;
    if TryStrToInt(param,result^.datoNumericoEntero) then begin {TryStrToInt: intenta convertir un string a entero. Recibe el string(param) y una aiable por referencia donde se guarda el entero}
        result^.datoNumericoEntero:= StrToInt(param);
        result^.datoNumericoReal:= result^.datoNumericoEntero;
        result^.esEntero:= true;
        result^.esReal:= true;
    end else if TryStrToFloat(param,result^.datoNumericoReal) then begin
        result^.datoNumericoEntero:= 0;
        result^.datoNumericoReal:= StrToFloat(param);
        result^.esEntero:= false;
        result^.esReal:= true;
    end else begin
        result^.datoNumericoEntero:= 0;
        result^.datoNumericoReal:= 0;
        result^.esEntero:= false;
        result^.esReal:= false;
    end;
end;
...
function esParametroString(const p: TParametro): boolean;
begin
    result:= (not p^.esEntero) and (not p^.esReal);
end;
...
function obtenerNumeroEntero(const p: TParametro): integer;
begin
  result:= p^.datoNumericoEntero;
end;

function obtenerNumeroReal(const p: TParametro): real;
begin
    result:= p^.datoNumericoReal;
end;
...
var i:byte;
begin
    for i:=1 to cantidadParametros(c) do begin
	    eliminarParametro(c^.listaparametros.argumentos[i]);
    end;
    dispose(c);
    c:= NIL;
end;
